    ...
    startInner() {
        ...
        for (let nest, isCovered; this.stoppedAt == null || cursor.from < this.stoppedAt;) {
            ...
            if (fragmentCursor.hasNode(cursor)) {
                if (overlay) {
                    let match = overlay.mounts.find((m) => m.frag.from <= cursor.from &&
                        m.frag.to >= cursor.to &&
                        m.mount.overlay);
                    if (match)
                ...
                if (typeof nest.overlay == "function") {
                    ...
                    if (ranges.length)
                        this.inner.push(new InnerParse(nest.parser, nest.parser.startParse(this.input, enterFragments(oldMounts, ranges), ranges), nest.overlay
...
function materialize(cursor) {
    ...
    function split(startI, endI, type, innerOffset, length) {
        ...
        children.push(isTarget
            ? node.toTree()
            : split(i + 4, b[i + 3], buf.set.types[b[i]], from, to - from));
